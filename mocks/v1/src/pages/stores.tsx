...
function storeHealth(d: ReturnType<typeof useDB>, s: SecretStore) {
  ...
  if (s.route && s.route !== 'public' && !conn) return { dot: 'error' as const, text: 'Vault connector revoked', tone: 'text-red-400' }
  if (conn?.health === 'offline') return { dot: 'degraded' as const, text: `Vault connector ${conn.name} offline`, tone: 'text-amber-400' }
...
export function StoreDetail() {
  ...
  const route = s.route === 'public' ? 'Reachable from the internet' : s.route ? `Through vault connector ${orgConnectors(d).find((c) => c.id === s.route)?.name ?? '(revoked)'}` : ''
