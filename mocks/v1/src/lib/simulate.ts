  ...
  update((d: DB) => {
    ...
    if (pick === ops) {
      ...
      log(d, {
        ...
        detail: [['Action', 'GET /v1/payouts'], ['Workspace', ws.name], ['Tool version', `v${eff === t ? t.version : t.published!.version}`]],
      })
      return
    }
...
export function testCall(opts: { toolId: string; wsId: string; actionId?: string; missingSlot?: string | null }) {
  const trk = trackingCode()
  const ms = 90 + Math.floor(Math.random() * 180)
  let result = { ok: true, status: 200, ms, trk, message: '' }
  update((d) => {
    const t = toolById(d, opts.toolId)
    ...
    log(d, {
      ...
      result: `200 · ${ms} ms`,
      trk,
      detail: [['Action', act ? `${act.method} ${act.path}` : '—'], ['Workspace', ws.name], ['Source', 'Test call']],
    })
  })
  return result
}
